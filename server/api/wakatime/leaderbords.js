...
const lineBot = (token, meesage) => {
  return axios.put(`${process.env.NOTIFY}line/${token}`, typeof meesage === 'string' ? { type: 'text', meesage } : meesage)
}
...
module.exports = {
  ...
  handler: async (req) => {
    ...
    lineBot(process.env.NODE_ENV == 'development' ? 'ris-robo/kem' : 'ris-robo/sru', flex).then(() => {
